...
module.exports = bot => {
  ...
  */
  ...
  app.get('/dashboard/:guildID/manage', checkAuth, async (req, res) => {
    ...
    const settings = await Server.findOne({ guildID: guild.id })
    renderTemplate(res, req, 'guild/manage.ejs', { guild, settings })
  })
  ...
  // otherwise it would be on the bot side and that would be horribly slow.
  ...
  app.post('/api/status', function (req, res) {
    res.sendStatus(200)
    console.log('Recieved post request on status webhook!')
    let pageJSON = req.body.page
    let incidentJSON = req.body.incident

    let statusEmbed = new Discord.MessageEmbed()
      .setAuthor(bot.user.username, bot.user.avatarURL)
      .setColor('#FF0000')
      .setTitle(pageJSON.status_description)
      .setDescription(`Current Status: ${incidentJSON.status}`)
      .setTimestamp()
      .addField('Impact Level', incidentJSON.impact, true)
      .addField('Incident', incidentJSON.name, true)

    if (incidentJSON.created_at !== undefined) {
      let createdAt = incidentJSON.created_at.split('-')
      statusEmbed.addField(
        'Incident Created at',
        `${createdAt[1]}-${createdAt[2].split('T')[0]}-${createdAt[0]} ${
          createdAt[2].split('T')[1]
        }`,
        true
      )
    }
    if (incidentJSON.scheduled_for != null) {
      statusEmbed.addField('Scheduled for', incidentJSON.scheduled_for, true)
    }
    if (incidentJSON.updated_at !== incidentJSON.created_at) {
      let updatedAt = incidentJSON.updated_at.split('-')
      statusEmbed.addField(
        'Incident Updated at',
        `${updatedAt[1]}-${updatedAt[2].split('T')[0]}-${updatedAt[0]} ${
          updatedAt[2].split('T')[1]
        }`,
        true
      )
    }
    if (incidentJSON.scheduled_until != null) {
      statusEmbed.addField(
        'Schedueled Until',
        incidentJSON.scheduled_until,
        true
      )
    }
    if (incidentJSON.resolved_at != null) {
      statusEmbed.addField('Resolved at', incidentJSON.resolved_at, true)
    }

    if (incidentJSON.incident_updates.length !== 0) {
      const bodies = []
      const updates = incidentJSON.incident_updates
      for (const update of updates) {
        bodies.push(`> ${update.body}`)
      }

      statusEmbed.addField('Updates', bodies.join('\n'))
    }

    bot.guilds.forEach(async guild => {
      const settings = bot.getSettings(guild.id)
      const logChannel = guild.channels.find('name', settings.logChannel)
      try {
        console.log(settings.logChannel)
        logChannel.send(statusEmbed)
      } catch (err) {
  ...
  // 404 on all routes that dont exist
